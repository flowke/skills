...
3. 当前需要进入 Intake、Truth 对齐、陪伴开发、Delivery Work-item、Regression 或记忆沉淀中的某一种工作模式。
...
## Core Concepts and Object Semantics
默认先建立下面这组最小概念，再进入规则与流程：

- `truth`：当前正在维护的核心事实锚点，不等于聊天结论的堆积。
- `current-truth.md`：truth 的主入口文档，用来承载当前对象状态、关键 truth items、恢复入口与下一步聚焦。
- `work-item`：正式的工作承接对象，用来承接对齐、规划、执行、验证、回写与归档；不等于狭义“待办卡片”。
- `knowledge`：跨 work-item 可复用、相对稳定的机制、经验、约束与判断。
- `intake`：尚未进入稳定对象层的原始输入、资料、链接与上下文。
- `对象落点`：判断一条信息最终应该进入 `intake / modules / topics / work-items / knowledge` 的哪一层。
...
4. **Delivery Work-item 模式**：当需要编排、恢复、多人 / 多轮续推或多步实施时，创建并推进 Delivery Work-item。
5. **Regression 模式**：基于 truth 检查当前代码是否一致，并判断是 `truth 过时`、`code 偏离` 还是 `暂时无法判断`。
...
- 承接 Delivery Work-item：包含实施路径选择、实施、必要验证 / 回归、结果回写与中断恢复。
- 承接 Regression：包含 truth 基线读取、回归检查、drift handling 分类与后续动作建议。
...
### Recovery-First and Truth Integrity
