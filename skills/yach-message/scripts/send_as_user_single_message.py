...
from yach_message_lib import (  # noqa: E402
    ...
    read_text_content,
    resolve_config,
    send_user_single_message,
)


def main():
    parser = argparse.ArgumentParser(description="Experimental: send a Yach/知音楼 message as a specific user")
    ...
    parser.add_argument("--to-user-id", required=True, help="Receiver Yach user ID, e.g. yach076429")
    ...
    try:
        ...
        result = send_user_single_message(
            config=config,
            token=token,
            message=message,
